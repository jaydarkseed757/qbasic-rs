...
SUB RiffleShuffle
   ' Card-riffle reshuffle visual around the deck pile at (568,224):
   ' split the pile into two halves, riffle a bridge of cards between
   ' them, then square them back up.  Drawn on the cleared felt.
   dckX = 568: dckY = 224
   lftX = 496                       ' resting x of the left half

   ' ---- phase 1: split the pile, sliding a half out to the left ----
   FOR x = dckX - 12 TO lftX STEP -12
      LINE (x + 56, dckY)-(x + 67, dckY + 79), 2, BF   ' erase trail at right
      DrawBackCard x, dckY
      DrawBackCard dckX, dckY                          ' keep the home half
      SOUND 1300 - (dckX - x), .8
   NEXT x

   ' ---- phase 2: riffle a bridge of cards across the gap ----
   FOR pass = 1 TO 2
      FOR x = lftX + 56 TO dckX STEP 4
         LINE (x, dckY + 4)-(x + 4, dckY + 73), 1, BF  ' card sliver body
         LINE (x, dckY + 4)-(x + 4, dckY + 4), 15      ' white top edge
         SOUND 1500 + INT(RND * 400), .5
         LINE (x, dckY)-(x + 4, dckY + 79), 2, BF      ' erase sliver
         DrawBackCard lftX, dckY                        ' keep both halves
         DrawBackCard dckX, dckY
      NEXT x
   NEXT pass

   ' ---- phase 3: square the halves back into one pile ----
   FOR x = lftX + 12 TO dckX STEP 12
      LINE (x - 12, dckY)-(x - 1, dckY + 79), 2, BF    ' erase trail at left
      DrawBackCard x, dckY
      DrawBackCard dckX, dckY
      SOUND 700 + (x - lftX), .8
   NEXT x
   DrawBackCard dckX, dckY
   SndShuffle                       ' rising flourish (the existing sound)
   COLOR 7
