...
REM $STATIC
'CalcDelay:
'  On native (non-DOS) hardware the busy-loop would spin billions of times
'  and return a huge MachSpeed, making Rest() wait for thousands of seconds.
'  Return SPEEDCONST directly so Rest() uses a 1:1 second-to-second mapping.
FUNCTION CalcDelay!
...
'Rest:
'  Pauses for t# seconds.  We compare TIMER directly (MachSpeed * t# /
'  SPEEDCONST == t# when CalcDelay returns SPEEDCONST) and call INKEY$ each
'  iteration so the OS event loop is pumped and the window keeps keyboard focus.
SUB Rest (t#)
  DIM s AS DOUBLE
  DIM Dummy AS STRING
  s = TIMER
