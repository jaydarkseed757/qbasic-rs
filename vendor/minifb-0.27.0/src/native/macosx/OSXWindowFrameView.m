...
- (void)drawInMTKView:(nonnull MTKView *)view {
    ...
    if (buffer_width > draw_state->texture_width || buffer_height > draw_state->texture_height) {
    ...
    MTLRegion region = {
        { 0, 0, 0 },
        { m_draw_parameters->buffer_width, m_draw_parameters->buffer_height, 1 }
    };
...
// On macOS 14+ NSView's default keyDown: calls interpretKeyEvents: which
// NSBeeps for printable characters and never reaches OSXWindow.keyDown:.
// Forward directly so key_callback fires for every keystroke.
- (void)keyDown:(NSEvent *)event {
    [[self window] keyDown:event];
}

- (void)keyUp:(NSEvent *)event {
    [[self window] keyUp:event];
}
